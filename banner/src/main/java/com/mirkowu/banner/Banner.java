...
import com.mirkowu.banner.loader.ImageLoaderInterface;
import com.mirkowu.banner.view.BannerViewPager;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    private boolean isAutoPlay = BannerConfig.IS_AUTO_PLAY;//autoPlay为true loop一定为true
    ...
    /**
     * 自动播放 为true 时  isloop 也为true 否则播放到末尾就会停止，不会循环
     * @param isAutoPlay
     * @return
     */
    public Banner isAutoPlay(boolean isAutoPlay) {
        this.isAutoPlay = isAutoPlay;
        if (isAutoPlay) this.isLoop = true;
        return this;
    }
    ...
    public Banner setPageClipToPadding(boolean clipToPadding) {
        viewPager.setClipToPadding(clipToPadding);
        return this;
    }

    public Banner setPageVerPadding(int verPadding) {
        viewPager.setPadding(viewPager.getPaddingLeft(), verPadding, viewPager.getPaddingRight(), verPadding);
        return this;
    }

    public Banner setPageHorPadding(int horPadding) {
        viewPager.setPadding(horPadding, viewPager.getPaddingTop(), horPadding, viewPager.getPaddingBottom());
        return this;
    }

    public Banner setPagePadding(int left, int top, int right, int bottom) {
    ...
    public Banner setCurrentItem(@IntRange(from = 0) int page) {
        if (count == 0)
            return this;
        if (page > count) {
...
//            if (imageLoader != null) {
//                imageLoader.displayImage(context, imageUrls.get(toRealPosition(position)), view);
//            } else {
//                Log.e(TAG, "Please set images loader.");
//            }
            //加载图片也可以放在这里进行
    ...
    public void onPageSelected(int position) {
        ...
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR ||
            ...
            if (isLoop) {
                ...
                indicatorImages.get(toRealPosition(position)).setImageResource(mIndicatorSelectedResId);
